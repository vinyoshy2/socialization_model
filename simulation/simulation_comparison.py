...
def produce_samples_beta(iter_results):
    ...
    for cur_iter in range(0, num_iters):
        for cur_vector in range(0, num_vectors):
            ...
            sampled_val = np.random.beta(a=params[1], b=params[0])
...
def compare_spec(ground_truth, iter_results, name):
    ...
    fig, axes = plt.subplots(nrows=num_vectors, ncols=1, figsize=(8, 15))
    for vec in range(num_vectors):
        ...
        g = sns.pointplot(ax=cur_ax, data=pd.DataFrame.from_records(records),
                      x="Index", y="Probability", hue="Type", errorbar=("pi", 95), linestyle=None)
        cur_ax.set_ylim(0, 1)
        cur_ax.set_title("{} {}".format(name, vec))
    plt.savefig("figures/{}_{}.pdf".format(folder, name))
    return fig
...
inferred_lambdas = produce_samples_beta(read3D("results/{}/lambda.txt".format(folder)))
...
compare_spec(gt_lambdas, inferred_lambdas, "lambdas")
